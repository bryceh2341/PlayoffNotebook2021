...
colnames(df)[which(names(df) == "Current Team")] <- "Team"

df <- df %>%
  separate(HT, c('feet', 'inches'), "-", convert = TRUE) %>% 
  mutate(HT = (12*feet + inches)) %>%
  select(Player, HT, Team)  
...
combined <- combined %>%
  arrange(Team) %>%
  mutate(Count = 1) %>%
  select(Team, MIN, HT, Count)

total <- ddply(combined,"Team",numcolwise(sum))
total <- total %>%
  mutate(weighted = Count/HT * MIN)

HTs = c(rep(total$MIN[1:30], total$Count[1:30]))

combined <- combined %>%
  mutate(Total = HTs) %>%
  mutate(Weight = round(MIN/Total * HT, 2)) %>%
  select(Team, Weight)
...
total %>%
  ...
  cols_label(Team = "Team",
             Weight = "Weighted Height (in.)",
  ) %>%
  tab_header(
    title = md("2021-22 Team Heights"),
