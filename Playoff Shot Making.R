...
pbp_team_PO %>% 
  ...
  tab_spanner(
    ...
    columns = vars(shot_quality_avg, efg_pct, shot_making, off_rtg)
  ) %>% 
  fmt_percent(
    columns = vars(shot_quality_avg, shot_making, efg_pct),
    decimals = 1
  )  %>%
  data_color(
  ...
  cols_width(vars(shot_quality_avg, efg_pct, shot_making, off_rtg) ~ px(45),
             vars(shot_quality_avg, efg_pct, shot_making, off_rtg) ~ px(30)) %>% 
  tab_style(
    style = list(
      cell_borders(
        side =  "top", 
