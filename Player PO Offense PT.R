...
All <- read.csv("Simmons.csv")

All <- All %>%
  select(ABB, PLAY_TYPE, POSS_PCT_R, POSS_PCT_PO, PPP_R, PPP_PO, PERCENTILE_R, PERCENTILE_PO)
All$ABB <- "PHI" 

All %>% 
  arrange(desc(POSS_PCT_PO)) %>% 
  gt()  %>% 
