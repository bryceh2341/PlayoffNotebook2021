...
combined <- combined %>%
  ...
  mutate(TIME_DIFF = POST_TIME - REG_TIME) %>%
  mutate(pos_neg = case_when(
    TIME_DIFF >= 0 ~ "Better On", 
    TRUE ~ "Worse on",))
...
comet_plot <- combined %>%
  ggplot() +
  geom_link(aes(x = REG_TIME, y = fct_reorder(PLAYER_NAME, TIME_DIFF), xend = POST_TIME, yend = fct_reorder(PLAYER_NAME, TIME_DIFF), color = pos_neg, size = stat(index))) +
  scale_color_manual(values = c("#00A087FF", "#E64B35FF")) +
  scale_size(range = c(.01, 4)) +
  scale_x_continuous(labels = c("0", "250", "500", "750", "1000"), breaks = seq(0, 1000, 250)) +
  theme_owen() +
  geom_point(
    data = filter(combined, TIME_DIFF > 0),
    aes(POST_TIME, y = fct_reorder(PLAYER_NAME, TIME_DIFF), color = pos_neg),
    shape = 21,
    fill = "white",
    size = 3.5
  )  +
  ...
  labs(x = "Possesion Time During Regular and Post Season",
       y = "",
       title = "Give Me the Ball!",
       subtitle = "Time Spent w/ the Ball for the Post Season vs. the Regular Season | Players Sorted by Differential")
# Save Plot
